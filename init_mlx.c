...
/*   Updated: 2023/10/06 18:42:05 by ojimenez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mouse_scroll_up(int param, int x, int y, t_fractal *f)
{
	...
	{
		f->zoom = f->zoom - 0.1;
		//recalculate_fract(f, f->zoom, f->x, f->y);
	}
	else if (param == 4)
	{
		f->zoom = f->zoom + 0.1;
...
void	init_mlx(t_fractal *f)
{
	...
	if (f->mlx == NULL)
		fr_destroy(f);
	f->win = mlx_new_window(f->mlx, WIDTH, HEIGHT, f->params[1]);
	if (f->win == NULL)
		fr_destroy(f);
	f->imag = mlx_new_image(f->mlx, WIDTH, HEIGHT);
	if (f->win == NULL)
		fr_destroy(f);
	f->timag = f->imag;
